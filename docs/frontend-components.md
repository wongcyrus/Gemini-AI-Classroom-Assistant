...
*   **`useClientLiteRTGemma.js` (Hook in `StudentView.jsx`)**: Custom hook executing on-device **Gemma LLM intent analysis** via Google LiteRT runtime (`litertGemma.worker.js`). Evaluates spoken transcripts for academic collusion (`COLLUSION_EXAM`), voice assistant dictation (`EXTERNAL_AI_ASSIST`), and unauthorized whispering (`UNAUTHORIZED_TALK`), while distinguishing legitimate technical questions. Automatically logs verified violations to `classes/{classId}/irregularities` with `source: 'on_device_gemma'` and updates real-time telemetry.
...
*   **`webAiModelLoader.js` / `webAiLiteRTLoader.js` / `gemmaLiteRTLoader.js` (Utilities)**: Edge AI utilities managing `CacheStorage` persistence (`webai-models-v1`, `webai-litert-whisper-v1`, `webai-litert-gemma-v1`), byte-level streamed progress calculations ($0\% \to 100\%$), and WebGPU/WASM hardware delegate resolution.
...
*   **`ControlsPanel.jsx`**: The consolidated sidebar control center for teachers during live monitoring. Reorganized in a logical top-down hierarchy:
  ...
  3. **👁️ AI & Invigilation**: Real-time mode indicators, gaze sensitivity summary, Gaze Configuration Modal launcher, class-wide **"⚡ Preload AI for All Students"** broadcast trigger, and Cloud Gemini Multimodal Analysis controls.
...
*   **`StudentScreen.jsx`**: A component used within `MonitorView.jsx` to display a single student's status, supporting split-dual viewports (side-by-side feeds) or single channel views with channel badges (🖥️ / 📷), live gaze orientation vectors, AI loading progress indicators (`⏳ 65%`), and multi-signal face status badges (`normal`, `looking_away`, `eyes_closed`, `talking`, `no_face`, `multiple_faces`, `cloud_fallback`).
