...
*   **`StudentScreen.jsx`**: A component used within `MonitorView.jsx` to display a single student's status, supporting split-dual viewports (side-by-side feeds) or single channel views with channel badges (🖥️ / 📷), offline frame indicator pills (`🖥️ Screen (Offline)` / `📷 Webcam (Offline)`), hardware absence indicators (📷🚫, 🎙️🚫), live gaze orientation vectors, AI loading progress indicators (`⏳ 65%`), live spoken transcript subtitles with language tag badges (`💬 粵`, `💬 普`, `💬 EN`), Gemma violation alert badges (`🚨 Collusion (Gemma)`), and multi-signal face status badges (`normal`, `looking_away`, `eyes_closed`, `talking`, `no_face`, `multiple_faces`, `cloud_fallback`).
...
*   **`useTeacherScreenBroadcast.js` / `TeacherScreenBroadcastModal.jsx` (Teacher Screen Sharing)**: Enables teachers to broadcast their screen (and optional audio) live in real time to all students in the class. Supports dual operational modes:
    *   **Classroom Frame Broadcaster Mode (Phase 2 Option A - Default)**: Scales seamlessly to 50+ students simultaneously with near-zero teacher CPU and zero extra server infrastructure. Captures screen frames into an offscreen canvas at 1.5s intervals, applies 32x18 thumbnail pixel diffing (skipping emissions if static unless 5s heartbeat expires), compresses clamped 720p frames at 0.65 JPEG quality (~35–65 KB, <8% of Firestore doc limit), and publishes to `classes/{classId}/screenBroadcast/liveFrame`. Eliminates all GPU encoding bottleneck and prevents browser lockups.
    *   **WebRTC Star Mesh Mode (Phase 1)**: 1-to-many WebRTC peer connection for low-latency video and audio streaming with clamped 720p @ 12 FPS, 350 kbps bitrate throttling, and hardware admission control (`MAX_ACTIVE_VIEWERS = 6`, queuing excess students to `status: 'queued'`).
    *   **Live Preview & Control Modal**: Displays active screen preview, mode badge (`🌐 Classroom Stream (50+ Students)` vs `⚡ WebRTC Star Mesh`), real-time viewer count, published frame counter, and roster of connected students.
*   **`useTeacherScreenBroadcastStudent.js` / `TeacherScreenViewerModal.jsx` (Student Screen Viewer)**: Enables students to watch the teacher's live screen broadcast seamlessly without interfering with background invigilation captures.
    *   **Automatic Mode Resolution**: Detects `broadcastMode` from session document. In Frame Broadcaster mode, subscribes to `liveFrame` doc snapshot and renders high-fidelity image stream; in WebRTC mode, establishes peer connection.
    ...
    *   **Hardware-Safe Non-Blocking Playback**: Runs independently from student screen/webcam proctoring streams, with mute toggle, connection state indicators, and automatic queued waiting banner when capacity limits apply.
