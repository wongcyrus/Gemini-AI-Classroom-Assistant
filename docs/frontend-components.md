...
*   **`useTeacherScreenBroadcast.js` / `TeacherScreenBroadcastModal.jsx` (Teacher Screen Sharing)**: Enables teachers to broadcast their screen (and optional audio) live in real time to all students in the class using an optimized 1-to-many WebRTC star topology with Firestore signaling (`classes/{classId}/screenBroadcast/session` and `classes/{classId}/screenBroadcastViewers/{studentUid}`).
    *   **Resolution & Framerate Clamping**: Clamps capture to 720p (1280x720) at max 12 FPS (`contentHint = 'detail'`) and throttles outbound video bitrate to 350 kbps via `RTCRtpSender.setParameters`, eliminating browser hangs and CPU/memory exhaustion.
    *   **Admission Control & Capacity Queuing**: Enforces a hardware encoder limit (`MAX_ACTIVE_VIEWERS = 6`). Additional student join requests are gracefully placed in a Firestore queue (`status: 'queued'`) and automatically promoted as active slots become available.
    *   **Real-Time Diagnostics & Candidate Deduplication**: Modal displays real-time slot counters (`Active Slots: X / 6`), queued viewer count, and deduplicates ICE candidates to prevent snapshot thrashing.
*   **`useTeacherScreenBroadcastStudent.js` / `TeacherScreenViewerModal.jsx` (Student Screen Viewer)**: Enables students to watch the teacher's live screen broadcast seamlessly without interfering with background invigilation captures. Features multiple layout modes (`🪟 Floating PiP`, `🔲 Standard Docked`, `⛶ Fullscreen`, `➖ Minimized Pill`), independent audio volume/mute toggles, connection state indicators, and an automatic queued waiting banner (`⏳ In Queue (Capacity Reached)`) when broadcast capacity is reached.
