...
*   **`faceLandmarker.worker.js` (Worker in `web-app/src/workers/`)**: Dedicated background Web Worker thread for MediaPipe model execution, image transfer via `ImageBitmap`, and geometric math, ensuring completely jank-free desktop UI performance.
*   **`webAiModelLoader.js` (Utility in `useFaceMonitor.js`)**: A dedicated edge AI loading utility that manages Cache API persistence for model weights, streams download chunks for real-time progress calculations, attempts GPU (WebGL/WebGPU) delegate allocation first, and gracefully degrades to CPU WASM binaries. Also exports geometric utilities `calculateEAR` and `calculateMAR`.
