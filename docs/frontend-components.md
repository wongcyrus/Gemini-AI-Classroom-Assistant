...
*   **`useTeacherScreenBroadcast.js` / `TeacherScreenBroadcastModal.jsx` (Teacher Screen Sharing)**: Enables teachers to broadcast their screen (and optional audio) live in real time to all students in the class using a 1-to-many WebRTC star topology with Firestore signaling (`classes/{classId}/screenBroadcast/session` and `classes/{classId}/screenBroadcastViewers/{studentUid}`). Includes a live broadcast preview modal with real-time viewer counters and connected student lists.
*   **`useTeacherScreenBroadcastStudent.js` / `TeacherScreenViewerModal.jsx` (Student Screen Viewer)**: Enables students to watch the teacher's live screen broadcast seamlessly without interfering with background invigilation captures. Features multiple layout modes (`🪟 Floating PiP`, `🔲 Standard Docked`, `⛶ Fullscreen`, `➖ Minimized Pill`), independent audio volume/mute toggles, and connection state indicators.
...
*   **`PerformanceAnalyticsView.jsx`**: Provides analytics and visualizations of student performance data. Features:
  ...
  * **Student Milestone Matrix**: Comprehensive tabular matrix tracking individual student completion times per milestone, total lab minutes, and mastery status.
  * **Dual CSV Export Capabilities**: Includes a top-level **`📥 Export CSV`** button for the full class milestone report, plus a dedicated **`📥 Export Matrix CSV ({count})`** button directly in the table header to export the actively searched/filtered student cohort.
...
*   **`VideoAnalysisJobs.jsx`**: Master dashboard for tracking, retrying, and synthesizing video analysis jobs.
  ...
  * **Full Prompt Inspector**: Inline expandable prompt accordion in Level 1 & Level 2, plus standalone **"📜 View Prompt"** modal (`PromptViewModal.jsx`) with 1-click clipboard copying.
*   **`VideoAnalysisJobsTable.jsx`**: Modular table sub-component for `VideoAnalysisJobs.jsx` rendering master jobs, status badges, multi-line prompt previews, and sub-job navigation (`View Details →`).
