...
*   **`useTeacherScreenBroadcast.js` / `TeacherScreenBroadcastModal.jsx` (Teacher Screen Sharing)**: Enables teachers to broadcast their screen live in real time to all students in the class using the **Classroom Frame Broadcaster** architecture.
    *   **Classroom Frame Broadcaster Architecture**: Scales seamlessly to 50+ students simultaneously with minimal teacher CPU, zero WebRTC encoder load, and zero extra server infrastructure. Captures screen frames into an offscreen canvas at 1.5s intervals, applies 32x18 thumbnail pixel delta diffing (skipping emissions if static unless 5s heartbeat expires), compresses clamped 720p frames at 0.65 JPEG quality (~35–65 KB, <8% of Firestore doc limit), and publishes to `classes/{classId}/screenBroadcast/liveFrame`. Eliminates all WebRTC encoder strain and completely prevents teacher browser lockups.
